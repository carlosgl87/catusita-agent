...
Recorre `clientes/agentes/*/` y lee su contrato público. El orquestador de este
multiagente solo puede delegar en estas áreas: las de `vendedores` no existen para él,
...
def _descubrir() -> dict:
    ...
    for info in pkgutil.iter_modules([str(_DIR)]):
        ...
        if any(not hasattr(mod, c) for c in _REQUERIDO):
            logging.warning(f"[clientes] área {info.name!r} sin contrato, ignorada.")
            continue
        areas[info.name] = mod
    return areas
