"""Descubrimiento de las áreas de Clientes.

Recorre `clientes/agentes/*/` y lee su contrato público. El orquestador de
este multiagente solo puede delegar en estas áreas: las de `vendedores` no existen
...
def _descubrir() -> dict:
    ...
    for info in pkgutil.iter_modules([str(_DIR)]):
        ...
        try:
            ...
            logging.error(f"[clientes] área {info.name!r} no importa: {e}")
            continue
        if faltan := [c for c in _REQUERIDO if not hasattr(mod, c)]:
            logging.warning(
                f"[clientes] área {info.name!r} sin {faltan}, ignorada.")
            continue
        areas[info.name] = mod
    return areas
...
def tools_de_delegacion() -> list:
    ...
    Quien hace el salto es el nodo `delegar` de `clientes/grafo.py`, que lee
    ...
    for nombre, mod in sorted(_AREAS.items()):
        if getattr(mod, "NODO", None) is None:
            ...
            logging.warning(f"[clientes] área {nombre!r} sin NODO, no se delega.")
            continue
