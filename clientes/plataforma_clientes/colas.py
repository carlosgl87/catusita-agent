"""La cola de clientes. Una sola.

    webhook ──Turno──► "clientes" ──► worker de clientes
...
COLA_RESPUESTAS = "respuestas:clientes"

# El servicio de Railway que levanta este worker.
SERVICIO = "catusita-clientes"
...
def problemas() -> list:
    ...
    for info in pkgutil.iter_modules([str(d)]):
        if not info.ispkg:
            continue
        try:
            m = importlib.import_module("clientes.agentes." + info.name)
        except Exception as e:
            fallos.append(f"clientes/agentes/{info.name} no importa: {e}")
            continue
        if getattr(m, "COLA", None):
            fallos.append(
                f"clientes/agentes/{info.name} declara COLA: las áreas no tienen "
