"""Padrón de clientes: le dice al router qué números son míos.

Al arrancar, este multiagente lee SU roster (`clientes`) y vuelca los números a
Redis. Eso es todo.

    clientes  ──(al prender)──►  Redis  hash "padron"  {numero: "clientes"}
                                            ▲
...
La fuente de verdad sigue siendo `clientes`, que no se toca acá.
...
async def publicar() -> int:
    ...
    Escribe SOLO sus propias entradas del hash: `HSET padron <numero> "clientes"`.
