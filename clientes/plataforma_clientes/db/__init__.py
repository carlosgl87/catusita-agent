...
El aislamiento NO lo da el pool, lo dan las tablas. `conocimiento_clientes` y
`conocimiento_vendedores` son tablas distintas, y cada área nombra solo la suya en
su `backend.py`. Con un pool compartido, `vendedores` sigue sin poder llegar a los
procesos de `clientes`: no tiene el nombre en ninguna parte de su paquete.
...
worker, a propósito, por tres razones:
  ...
  1. HAY UNA SOLA BASE Y UN SOLO ESQUEMA. Las tablas están separadas por
     multiagente —`chat_messages_clientes`, `conocimiento_clientes`— pero
     viven en la misma BD, y `migraciones` es una sola tabla para todas. Una
     migración no es «de clientes»: la 008 renombró columnas en las tres
...
resolvía a `clientes/db/migrations`, que no existe — y `glob()` sobre un
directorio inexistente devuelve vacío. Corría cero migraciones en silencio. Y
aunque la ruta hubiera estado bien, el Dockerfile copia solo `clientes/`, así
