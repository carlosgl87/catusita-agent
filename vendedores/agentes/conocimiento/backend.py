...
`conocimiento_vendedores` y `solicitud_proceso_nuevo_vendedores`
(ver db/migrations/004_conocimiento.sql — SIN APLICAR).

Ya NO lleva filtro por multiagente. La tabla ES la de vendedores: la frontera dejó de
...
TABLA_SOLICITUD  = "solicitud_proceso_nuevo_vendedores"
...
async def similares(vector: list[float], k: int = 5) -> list[dict]:
    """Top-k de `conocimiento_vendedores`. [{titulo, cuando, pasos, entrega, similitud}]"""
...
async def registrar_solicitud(
    ...
    """Deja constancia de un proceso que falta, en `solicitud_proceso_nuevo_vendedores`.
