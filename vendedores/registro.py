...
Recorre `vendedores/agentes/*/` y lee su contrato público. El orquestador de
...
def _descubrir() -> dict:
    ...
    for info in pkgutil.iter_modules([str(_DIR)]):
        ...
        if faltan := [c for c in _REQUERIDO if not hasattr(mod, c)]:
            logging.warning(
                f"[vendedores] área {info.name!r} sin {faltan}, ignorada.")
            continue
        areas[info.name] = mod
    return areas
...
def tools_de_delegacion() -> list:
    ...
    Quien hace el salto es el nodo `delegar` de `vendedores/grafo.py`, que lee
    ...
    for nombre, mod in sorted(_AREAS.items()):
        if getattr(mod, "NODO", None) is None:
            ...
            logging.warning(f"[vendedores] área {nombre!r} sin NODO, no se delega.")
            continue
