...
Migrado desde orchestrator/nodes/validar.py, con SOLO las reglas de vendedores.
"""
import json
import logging
import re
...
from vendedores.plataforma_vendedores import llm
...
# ─── Reglas de privacidad de vendedores ──────────────────────────────────
#
...
async def nodo_validar(state: EstadoAgente) -> dict:
    ...
    def rechazar(motivo: str) -> dict:
        logging.warning(f"validar RECHAZÓ (vendedores, intento {intentos}): {motivo[:100]}")
