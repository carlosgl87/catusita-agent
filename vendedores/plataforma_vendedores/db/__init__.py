...
worker, a propósito, por tres razones:
  ...
  1. HAY UNA SOLA BASE Y UN SOLO ESQUEMA. Las tablas están separadas por
     multiagente —`chat_messages_vendedores`, `conocimiento_clientes`— pero
     viven en la misma BD, y `migraciones` es una sola tabla para todas. Una
     migración no es «de vendedores»: la 008 renombró columnas en las tres
...
resolvía a `vendedores/db/migrations`, que no existe — y `glob()` sobre un
directorio inexistente devuelve vacío. Corría cero migraciones en silencio. Y
aunque la ruta hubiera estado bien, el Dockerfile copia solo `vendedores/`, así
