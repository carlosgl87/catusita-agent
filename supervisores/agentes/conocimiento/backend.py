...
`conocimiento_supervisores` y `solicitud_proceso_nuevo_supervisores`
(ver db/migrations/004_conocimiento.sql — SIN APLICAR).

Ya NO lleva filtro por multiagente. La tabla ES la de supervisores: la frontera dejó de
...
proceso de otro multiagente no puede salir de acá ni con el query mal escrito.
    ...
    SELECT titulo, cuando, pasos, entrega, 1 - (embedding <=> $1) AS similitud
      FROM conocimiento_supervisores
     WHERE activo
...
TABLA_SOLICITUD  = "solicitud_proceso_nuevo_supervisores"
...
async def similares(vector: list[float], k: int = 5) -> list[dict]:
    """Top-k de `conocimiento_supervisores`. [{titulo, cuando, pasos, entrega, similitud}]"""
...
async def registrar_solicitud(
    ...
    """Deja constancia de un proceso que falta, en `solicitud_proceso_nuevo_supervisores`.
