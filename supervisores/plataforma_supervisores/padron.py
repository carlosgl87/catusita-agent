"""Padrón de supervisores: le dice al router qué números son míos.

Al arrancar, este multiagente lee SU roster (`supervisores`) y vuelca los números a
Redis. Eso es todo.

    supervisores  ──(al prender)──►  Redis  hash "padron"  {numero: "supervisores"}
                                            ▲
...
La fuente de verdad sigue siendo `supervisores`, que no se toca acá.
...
async def publicar() -> int:
    ...
    Escribe SOLO sus propias entradas del hash: `HSET padron <numero> "supervisores"`.
    Los de otros multiagentes viven en el mismo hash y no se tocan.
    """
    pool = await db.get()
    async with pool.acquire() as c:
        filas = await c.fetch(
            """SELECT whatsapp FROM supervisores
