...
Migrado desde orchestrator/nodes/validar.py, con SOLO las reglas de supervisores.
"""
import json
import logging
import re
...
from supervisores.plataforma_supervisores import llm
...
# ─── Reglas de privacidad de supervisores ────────────────────────────────
#
...
# Un supervisor ve lo mismo que sus asesores. Se revisa cuando se defina su
# alcance (ver supervisores/orquestador.py).
_PROHIBIDO = [
...
async def nodo_validar(state: EstadoAgente) -> dict:
    ...
    def rechazar(motivo: str) -> dict:
        logging.warning(f"validar RECHAZÓ (supervisores, intento {intentos}): {motivo[:100]}")
